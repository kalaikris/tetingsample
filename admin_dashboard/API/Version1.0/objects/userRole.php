...
class userRole extends Database{
    function userRoleNameCheck(){
        $query = "SELECT `token` FROM `admin__user_roles` 
        WHERE `name`=:name 
        AND `status`='1'";
    ...
    function addUserRole(){
        $query = "INSERT INTO `admin__user_roles` SET 
        `token`=:token,
        `name`=:name,
    ...
    function addUserModules(){
        ...
        $query = "INSERT INTO `admin__user_roles_modules`(
        `role_token`, 
        `module_token`, 
        `status`, 
        `created_date`
    ...
    function updateUserRole(){
        $query = "UPDATE `admin__user_roles` SET 
        `name`=:name
        WHERE `token`=:token";
        $stmt = $this->conn->prepare( $query );
        $stmt->bindParam('name', $this->userRoleName);
    ...
    function updateUserModules(){
        $query1 = "UPDATE `admin__user_roles_modules` SET 
        `status`='2'
        ...
        foreach($array as $value){
            ...
            if($checkStmt->rowCount()==0){
                $query = "INSERT INTO `admin__user_roles_modules` SET
                ...
                $query = "UPDATE `admin__user_roles_modules` SET 
                `status`='1'
                WHERE `role_token`=:user_role_token
    ...
    function checkRoleModule($moduleToken){
        $query = "SELECT `id` 
        FROM `admin__user_roles_modules` WHERE `role_token`=:user_role_token 
        AND `module_token`=:module_token";
        $stmt = $this->conn->prepare( $query );
        $stmt->bindParam('user_role_token', $this->userRoleToken);
    ...
    function userRoleList(){
        ...
        FROM `admin__user_roles`  AS `user_role`
        INNER JOIN `admin__user_roles_modules` AS `user_role_module` ON
        (
            `user_role_module`.`role_token`=`user_role`.`token`
            AND `user_role_module`.`status`='1'
        )
        INNER JOIN `admin__modules` AS `modules` ON `modules`.`token`=`user_role_module`.`module_token` 
