...
if($stmt->rowCount()==1){
    ...
    if($inputData->fromDate != "" && $inputData->toDate!=""){
        $dateQuery = " AND `users__booking_detail`.`service_completed_date_time` BETWEEN '$fromDate' AND '$toDate'";
    }
    $report->dateQuery  = $dateQuery;
    $stmt        = $report->distributorCommissionReport();
    $obj->report = $report->viewDistributorCommissionReport($stmt);
}else{
    $obj->status_code = 503;
    $obj->title       = "Error";
    $obj->message     = "Error";
