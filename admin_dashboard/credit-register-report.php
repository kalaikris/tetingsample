...
<body>
    ...
    <main class="main-contents">
        <section class="bg-white full-height" id="toggle3">
            <div class="header-details">
                <h1 class="header_main"> Credit Reports</h1>
                ...
                <table class="custom-table booking-manage" id="dataTables_filter">
                    <thead>
                        <tr>
                            ...
                            <th>Service Type</th>
                            <th>HSN CODE</th>
                            <th>Total value of Services Cancelled</th>
                            <th>Cancellation Charges</th>
                            <th>Amount Refundable to customer</th>
                            <th>Taxable value of Cancelled Services </th>
                            <th>GST on cancelled Services</th>
                            <th>Total CN Value </th>
    ...
    <script>
        ...
        $(document).ready(() => {
            ...
            fetchcreditreport(onloadfromDate,onloadtoDate);
        });

        function fetchcreditreport(fromdate,todate){
            let datas = {
            ...
            let json1 = JSON.stringify(datas);
                    ...
                    dataType: "JSON",
                    type: "POST",
                    url: apiPath+"/reports/creditNoteRegister.php",
                    data: json1
                    }).done(function(data1) {
                        let creditReportArray = data1.report;
                        let reportlist = '';
                        creditReportArray.forEach((reports,index) => {
                            reportlist += `<tr>
                                            <td>${index + 1}</td>
                                            <td>${reports.creditNumber}</td>
                                            <td>${reports.cnDate}</td>
                                            <td>${reports.invoiceNumber}</td>
                                            <td>${reports.airportzo_gst_no}</td>
                                            ...
                                            <td>${reports.gstin_number}</td> 
                                            <td>${reports.serviceType}</td>
                                            <td>${reports.hsnCode}</td>
                                            <td>${reports.totalValue}</td>
                                            <td>${reports.cancellationFee}</td>
                                            <td>${reports.refundableAmount}</td>
                                            <td>${reports.taxableValue}</td>
                                            <td>${reports.gstOnCancelledService}</td>
                                            <td>${reports.refundableAmount}</td>
                                            <td>${reports.refundId}</td>
                                        </tr>`;
                        });
                        ...
                        $("#dataTables_filter").DataTable({
                            ...
                            buttons: [
                                {
                                    extend: 'csvHtml5',
                                    title: 'Admin Distributor Reports'
                                },
                                {
                                    extend: 'pdfHtml5',
        ...
        $('body').on('click','.daterangefilter',function(){
            ...
            if(fromfullDate <= tofullDate && fromDate != '' && toDate != ''){
                ...
                fetchcreditreport(fromDate,toDate);
            }else{
