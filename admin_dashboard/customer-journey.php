...
<body>
    ...
    <main class="main-contents">
        <section class="bg-white full-height" id="toggle3">
            <div class="header-details">
                <h1 class="header_main">Customer Journey Reports</h1>
                ...
                <table class="custom-table booking-manage" id="dataTables_filter">
                    <thead>
                        <tr>
                            ...
                            <th>GST on convenience fee</th>
                            <th>Total Revenue</th>
                            <th>Cancellation Base Value</th>
                            <th>Cancellation GST</th>
                            <th>Total Cancellation Value</th>
                            <th>Distributor Commsission excl. GST</th>
                            <th>Commission GST</th>
                            <th>Total Distributor Commission</th>
                            <th>Provider Commsission excl. GST</th>
                            <th>Commission GST</th>
                            <th>Total Provider Commission</th>
                            <th>Profit</th>
                            <th>Cash Flow</th>
                            <th>Membership Id</th>
                        </tr>
    ...
    <script>
        ...
        $(document).ready(() => {
            ...
            fetchcustomerjourneyreport(onloadfromDate,onloadtoDate);
        });
        function fetchcustomerjourneyreport(fromdate,todate){
            ...
            let json1 = JSON.stringify(datas);
                    ...
                    dataType: "JSON",
                    type: "POST",
                    url: apiPath+"/reports/customerJourney.php",
                    data: json1
                    }).done(function(data1) {
                        let CustomerJourneyReportArray = data1.report;
                        let reportlist = '';
                        CustomerJourneyReportArray.forEach((reports,index) => {
                            reportlist += `<tr>
                                            <td>${index + 1}</td>
                                            <td>${reports.customerName}</td>
                                            <td>${reports.serviceDistributorName}</td>
                                            <td>${reports.serviceProviderName}</td>
                                            <td>${reports.bookingDate}</td>
                                            <td>${reports.serviceDate}</td>
                                            <td>${reports.bookingRef}</td>
                                            <td>${reports.serviceRef}</td>
                                            <td>${reports.status}</td>
                                            <td>${reports.cancelledDate}</td>
                                            <td>${reports.businessType}</td>
                                            <td>${reports.revenueBaseValue}</td>
                                            <td>${reports.revenueGstValue}</td>
                                            <td>${reports.revenueSaleValue}</td>
                                            <td>${reports.convenienceFee}</td>
                                            <td>${reports.convenienceFeeGst}</td>
                                            <td>${reports.totalRevenue}</td>
                                            <td>${reports.cancelBaseValue}</td>
                                            <td>${reports.cancelGstValue}</td>
                                            <td>${reports.cancelSaleValue}</td>
                                            <td>${reports.distCommissionExclGstValue}</td>
                                            <td>${reports.distCommissionGstValue}</td>
                                            <td>${reports.distCommissionTotalValue}</td>
                                            <td>${reports.payToServiceProviderBaseVal}</td>
                                            <td>${reports.payToServiceProviderGstValue}</td>
                                            <td>${reports.payToServiceProviderTotalVal}</td>
                                            <td>${reports.profit}</td>
                                            <td>${reports.cashFlow}</td>
                        ...
                        $("#dataTables_filter").DataTable({
                            ...
                            buttons: [
                                {
                                    extend: 'csvHtml5',
                                    title: 'Customer Journey Reports'
                                },
                                {
                                    extend: 'pdfHtml5',
        ...
        $('body').on('click','.daterangefilter',function(){
            ...
            if(fromfullDate <= tofullDate && fromDate != '' && toDate != ''){
                ...
                fetchcustomerjourneyreport(fromDate,toDate);
            }else{
