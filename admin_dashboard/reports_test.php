...
<body>
                ...
                <table class="custom-table booking-manage" id="dataTables_filter">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>Service Distributor/Web/App/Ios/Android App</td>
                            <td>DEL Terminal 3</td>
                            <td>Mr. Bithal Das Mundhra</td>
                            <td>
                               <p>Moonthal</p> 
                               <p>91 8610725198</p>
                               <p>support@airporto.com</p>
                            </td>
                            <td>4</td>
                            <td>4</td>
                            <td>4</td>
    ...
    <script>
        ...
        $(document).ready(() => {
            ...
            fetchdistributorreport(onloadfromDate,onloadtoDate);
        });

        function fetchdistributorreport(fromdate,todate){
            let datas = {
            ...
            let json1 = JSON.stringify(datas);
                    ...
                    dataType: "JSON",
                    type: "POST",
                    url: apiPath+"/reports/allServiceDistributorData.php",
                    data: json1
                    }).done(function(data1) {
                        console.log(data1);
                        let distributorReportArray = data1.data;
                        $('.total').text(`Total Distributor Reports - ${distributorReportArray.length}`);
                        let reportlist = '';
                        distributorReportArray.forEach((reports,index) => {
                            reportlist += `<tr>
                                            <td>${index + 1}</td>
                                            <td>${reports.status}</td>
                                            <td>${reports.name}</td>
                                            <td><table><tr>${reports.address}</tr>
                                            <tr>hii</tr></table></td>
                                            <td>${reports.primary_email}</td>
                                            <td>${reports.mobile_number}</td>
                                            <td>${reports.email}</td>
                                            <td>${reports.alternate_mobile_number}</td>
                                            <td>${reports.account_number}</td>
                                            <td>${reports.ifsc_code}</td>
        ...
        $('body').on('click','.daterangefilter',function(){
            ...
            if(fromfullDate <= tofullDate && fromDate != '' && toDate != ''){
                ...
                fetchdistributorreport(fromDate,toDate);
            }else{
