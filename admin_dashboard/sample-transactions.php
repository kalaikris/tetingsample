...
<body>
                ...
                <table class="custom-table booking-manage" id="dataTables_filter">
                    <thead>
                        <tr>
                            ...
                            <th>Additional Child</th>
                            <th>Adult Regular Price</th>
                            <th>Markup</th>
                            <th>Adult Price</th>
                            <th>Child Regular Price</th>
                            <th>Markup</th>
                            <th>Child Price</th>
                            <th>Additional Adult Regular Price</th>
                            <th>Markup</th>
                            <th>Additional Adult Price</th>
                            <th>Additional Child Regular Price</th>
                            <th>Markup</th>
                            <th>Additional Child Price</th>
                            <th>Service Regular Value</th>
                            <th>Service Value Markup</th>
                            <th>Service Value Incl GST</th>
                            <th>Trade Discount</th>
                            <th>Service Value Post Discount</th>
                            <th>Agent Convinience Fee</th>
                            <th>Convenience  fee</th>
                            <th>Gst On Con Fee</th>
    ...
    <script>
        ...
        $(document).ready(() => {
            ...
            fetchtransactionreport(onloadfromDate,onloadtoDate);
        });
        function fetchtransactionreport(fromdate,todate){
            let radioBtn = $("input[name='date_filter']:checked").val();
            let datas = {
            ...
            let json1 = JSON.stringify(datas);
                    $.ajax({
                        dataType: "JSON",
                        type: "POST",
                        url: apiPath+"/reports/transactionReport.php",
                        data: json1
                    }).done(function(data1) {
                        let transactionReportArray = data1.report;
                        $('.total').text(`Total Reports - ${transactionReportArray.length}`);
                        let reportlist = '';
                        transactionReportArray.forEach((reports,index) => {
                            reportlist += `<tr>
                                            <td>${index + 1}</td>
                                            <td>${reports.customerName}</td>
                                            <td>${reports.serviceDistributorName}</td>
                                            <td>${reports.serviceProviderName}</td>
                                            <td>${reports.agentName}</td>
                                            <td>${reports.gstNumber}</td>
                                            <td>${reports.bookedDate}</td>
                                            <td>${reports.serviceDate}</td>
                                            <td>${reports.status}</td>
                                            <td>${reports.bookingNumber}</td>
                                            <td>${reports.token}</td>
                                            <td>${reports.serviceName}</td>
                                            <td>${reports.totalAdult}</td>
                                            <td>${reports.totalChildren}</td>
                                            <td>${reports.additionalAdult}</td>
                                            <td>${reports.additionalChildren}</td>
                                            <td>${reports.adultServiceAmount}</td>
                                            <td>${reports.adult_price_markup}</td>
                                            <td>${reports.adult_price_after_markup}</td>
                                            <td>${reports.childrenServiceAmount}</td>
                                            <td>${reports.child_price_markup}</td>
                                            <td>${reports.child_price_after_markup}</td>
                                            <td>${reports.additionalAdultServiceAmount}</td>
                                            <td>${reports.add_adult_price_markup}</td>
                                            <td>${reports.add_adult_price_after_markup}</td>
                                            <td>${reports.additionalXhildrenServiceAmount}</td>
                                            <td>${reports.add_child_price_markup}</td>
                                            <td>${reports.add_child_price_after_markup}</td>
                                            <td>${reports.service_value_before_markup}</td>
                                            <td>${reports.service_value_markup}</td>
                                            <td>${reports.netAmount}</td>
                                            <td>${reports.trade_discount}</td>
                                            <td>${reports.serv_val_post_discount}</td>
                                            <td>${reports.agent_convin_fee}</td>
                                            <td>${reports.convenienceFee}</td>
                                            <td>${reports.convenienceFeeGst}</td>
                                            <td>${reports.totalInvoice}</td>
                                            <td>${reports.paymentId}</td>
                                            <td>${reports.assignedTo}</td>
                                            <td>${reports.membershipNumber}</td>
                                        </tr>`;
                        });
                        ...
                        $("#dataTables_filter").DataTable({
                            ...
                            buttons: [
                                {
                                    extend: 'csvHtml5',
                                    title: 'Admin Transaction Reports'
                                },
                                {
                                    extend: 'pdfHtml5',
