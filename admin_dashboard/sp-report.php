...
<body>
    ...
    <main class="main-contents">
        <section class="bg-white full-height" id="toggle3">
            <div class="header-details">
                <h1 class="header_main">Service Provider Reports</h1>
                ...
                <table class="custom-table booking-manage" id="dataTables_filter">
                    <thead>
                        <tr>
                            ...
                            <th>Applicable Cancellation Policy</th>
                            <th>Cancellation Charge Percentage</th>
                            <th>Provider Name</th>
                            <th>Service Type</th>
                            <th>Adult</th>
                            <th>Child</th>
                            ...
                            <th>Service Provider B2C RATE</th>
                            <th>AZ Commission</th>
    ...
    <script>
        ...
        $(document).ready(() => {
            ...
            fetchspreport(onloadfromDate,onloadtoDate);
        });
        function fetchspreport(fromdate,todate){
            let datas = {
            ...
            let json1 = JSON.stringify(datas);
                    ...
                    dataType: "JSON",
                    type: "POST",
                    url: apiPath+"/reports/spReport.php",
                    data: json1
                    }).done(function(data1) {
                        let spReportArray = data1.report;
                        let reportlist = '';
                        spReportArray.forEach((reports,index) => {
                            reportlist += `<tr>
                                            <td>${index + 1}</td>
                                            ...
                                            <td>${reports.cancellationHours}</td>
                                            <td>${reports.cancellationPercentage}</td>
                                            <td>${reports.providerName}</td>
                                            <td>${reports.businessType}</td>
                                            <td>${reports.adult}</td>
                                            <td>${reports.child}</td>
                                            <td>${reports.additionalAdult}</td>
                                            <td>${reports.additionalChild}</td>
                                            <td>${reports.priceAdult}</td>
                                            <td>${reports.priceChildren}</td>
                                            <td>${reports.additionalPriceAdult}</td>
                                            <td>${reports.additionalPriceChildren}</td>
                                            <td>${reports.service_provider_B2C}</td>
                                            <td>${reports.AZ_commission}</td>
                                            <td>${reports.after_commission}</td>
                                            <td>${reports.servicePayable}</td>
