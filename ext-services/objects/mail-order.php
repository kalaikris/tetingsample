...
class MailTemplateOrder {
    ...
    public function genMailContentForAdminAndUser() {
        $distributor_logo = 'https://airportzostage.in/service-distributor-dashboard/asset/img/logo.png'; //($this->mail_obj->header_logo != '')? $this->mail_obj->header_logo: 'https://airportzostage.in/service-distributor-dashboard/asset/img/logo.png';
        $brand_color = '#07B4D3'; //($this->mail_obj->brand_colour != '')? $this->mail_obj->brand_colour: '#07B4D3';
        $passenger_array = [];
        ...
        $departure_dates = [];
        foreach ($this->mail_obj->journey_array as $journey_key => $journey_value) {
        ...
        $mail_template = '<body>
                ...
                foreach ($this->mail_obj->station_array as $station_value) {
                    $service_view_array = [];
//                    $cancellation_view = [];
                    foreach ($station_value->service_array as $service_value) {
                        $service_passenger_array = [];
                        if ($service_value->price_detail->total_adult > 0) {
                            array_push($service_passenger_array, $service_value->price_detail->total_adult . " Adults");
                        }
                        if ($service_value->price_detail->total_children > 0) {
                            array_push($service_passenger_array, $service_value->price_detail->total_children . " Children");
                        }

                        $company_logo = ($service_value->service_provider->logo != '')? $service_value->service_provider->logo: "https://airportzo.net.in/invoince-template/product-logo.png";
                        //praveen
...
//                                        <span style="display:block;font-size: 22px;line-height: 30px;font-family: sans-serif;color: #242424;text-align: left;padding-bottom: 5px;font-weight: 700;">' . $service_value->service_provider->name . '</span>
//                                        <span style="display:block;font-size: 18px;line-height: 20px;font-family: sans-serif;color: #8f8b8b;text-align: left;padding-bottom: 5px;">Order ID : ' . $service_value->order_id . '</span>
//                                        <span style="display: flex;justify-content: space-between;">
//                                            <span style="display:block;font-size: 18px;line-height: 20px;font-family: sans-serif;color: #8f8b8b;text-align: left;padding-bottom: 5px;margin-right: 12px;">' . $service_value->service_name . ' | ' . implode(" & ", $service_passenger_array) . '</span>
//                                            <span style="display:block;font-size:22px;line-height:24px;font-family: sans-serif;color: #242424;text-align: left;margin-left:auto;white-space: nowrap;">INR ' . $service_value->price_detail->net_amount . '</span>
//                                        </span>
//                                    </span>
//                                </li>
//                            </ul>');
                        $service_views = '<ul style="display: flex;list-style-type: none;margin: 0px;padding: 0px;padding-top: 30px;width: 100%;">
                                <li style="width: 100%;display: flex;">
                                    <span style="display: block;margin-right:10px;">
                                        <img src="' . $company_logo . '" alt="" style="width: 55px;">
                                    </span>
                        ...
                        //cancell
                        $cancellation_view = [];
                        $cancellation_count = sizeof($service_value->cancellation_policy_detail);
                        $cancel_policy_array = $service_value->cancellation_policy_detail;
                        $min_hr = 0;
                        foreach ($cancel_policy_array as $cp_value) {
                            if ($cp_value->cancellation_fee_percentage == 0) {
                                $cp_value->refund = "Full";
                            } else if ($cp_value->cancellation_fee_percentage == 100) {
                                $cp_value->refund = "No";
                            } else {
                                $cp_value->refund = (100 - (int) $cp_value->cancellation_fee_percentage) . "%";
                            }
                            $cp_value->is_before = true;
                            if ($min_hr == 0 || $min_hr > $cp_value->hours_before_cancelling) {
                                $min_hr = $cp_value->hours_before_cancelling;
                            }
                        } unset($cp_value);
                        if ($min_hr > 0) {
                            $cancel_obj = new stdClass;
                            $cancel_obj->hours_before_cancelling = $min_hr;
                            $cancel_obj->cancellation_fee_percentage = 100;
                        ...
                        foreach ($cancel_policy_array as $cp_key => $cp_value) {
                            if ($cp_key % 2 == 0) {
                                $temp_td_arr = [];
                                $hour_view = ($cp_value->is_before)? $cp_value->hours_before_cancelling . ' hours before': 'After ' . $cp_value->hours_before_cancelling . ' hours';
                            ...
                            //     array_push($cancellation_view, $tr);
                            // }
                            unset($cancel_policy_array[$cp_key]->refund);
                            unset($cancel_policy_array[$cp_key]->is_before);
                        } unset($cp_value);
                        $service_footer = "";
                        if (sizeof($cancellation_view) > 0) {
                            $service_footer .= '<div style="margin-bottom: 16px;background-color: #f6f6f6;border-radius: 12px;padding: 12px;">
                        ...
                        if (property_exists($service_value, 'reschedule_policy')) {
                            if ($service_value->reschedule_policy != '') {
                                $service_footer .= '<div style="">
                                                        ...
                                                        <p style="color:#4b4b4b;font-size: 14px;font-family: "Rubik",sans-serif;padding-left:25px;margin:0 0 8px;line-height: 20px;">' . $service_value->reschedule_policy . '
                                                        </p>
                                                    </div>';
                            }
                        }
                        ...
                        $service_data = $service_views.$service_footer;
                      array_push($service_view_array, $service_data); 
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <span>
                                                                ...
                                                                <span style="display:block;font-size:18px;line-height:20px;font-family: sans-serif;color: #8f8b8b;text-align: left;font-weight: 700;">' . $station_value->service_array[0]->service_date_time . '</span>
                                                            </span>
            ...
            $mail_template .= '</table>
                ...
                foreach ($this->mail_obj->journey_array as $journey_key => $journey_value) {
                    if ($journey_key%2 == 0) {
                        ...
                        if ($journey_key < sizeof($this->mail_obj->journey_array)-1) {
                            $flight_number = ($this->mail_obj->journey_array[$journey_key + 1]->flight_number != '')? $this->mail_obj->journey_array[$journey_key + 1]->flight_number . '[' . $this->mail_obj->journey_array[$journey_key + 1]->depart_date . ']': '-';
                            $mail_template .= '<td style="padding-bottom: 10px;">
                                                <span style="display:block;font-size: 18px;line-height: 20px;font-family: sans-serif;color: #8f8b8b;text-align: left;padding-bottom: 5px;">' . $this->mail_obj->journey_array[$journey_key + 1]->depart_airport_code . ' Flight Number</span>
                                                <span style="display:block;font-size:22px;line-height:24px;font-family: sans-serif;color: #242424;text-align: left;padding-bottom: 3px;">' . $flight_number . '</span>
                                            </td>';
                        }
                        $mail_template .= '</tr>';
            ...
            if ($this->mail_obj->notes != '') { // } || $this->mail_obj->description_two != '') {
                    $mail_template .= '<tr>
                    <td>
                        <hr style="border-top: 1px solid #f2f2f2;width: 90%;margin: auto;">
                    </td>
                </tr>';
                if ($this->mail_obj->notes != '') {
                    $mail_template .= '<tr>
                        <td>
                            <table style="width: 100%;padding: 20px 30px;">
                                <tbody>
                                    <tr>
                                        <td>
                                            ...
                                            <span style="display:block;font-size:22px;line-height:30px;font-family: sans-serif;color: #242424;text-align: left;padding-bottom: 3px;">' . $this->mail_obj->notes . '</span>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
...
//                                                    <span style="display:block;font-size:18px;line-height:20px;font-family: sans-serif;color: #8f8b8b;text-align: left;font-weight: 700;">Cancellation Policysss</span>    
//                                                </span>
    ...
    public function genMailContentForServiceProvider() {
        ...
        $mail_template = '<table style="width:800px;margin:auto;box-shadow: 1px 1px 20px 2px #00000029;border-radius: 12px;overflow: hidden;background: #fbfbfb;">
                <thead>
                    ...
                    // echo json_encode($this->mail_obj);
        foreach ($this->mail_obj->station_array as $station_key => $station_value) {
            $flight_number = "";
            $temp_journey_array = $this->mail_obj->journey_detail;
            if ($station_key) {
                if ($station_key == sizeof($temp_journey_array)+1) {
                    $flight_number = $temp_journey_array[$station_key-1]->flight_number;
                } else {
                    $flight_number = $temp_journey_array[$station_key-1]->flight_number . ", " . $temp_journey_array[$station_key]->flight_number;
                }
            } else {
                $flight_number = $temp_journey_array[$station_key]->flight_number;
            }

            // echo json_encode($station_value);
            $mail_template .= '<tr>
                        <th>
                                        ...
                                        <td>
                                            <span style="display: flex;align-items:center;margin: 10px auto;">
                                                ...
                                                <span>
                                                    ...
                                                    <span style="display:block;font-size: 24px;line-height: 30px;font-family: sans-serif;color: #242424;text-align: left;font-weight: 700;">' . $station_value->airport_name . '</span>
                                                </span>
                                            </span>
                                        </td>
                                        <td>
                                            <span style="display: flex;align-items:center;margin: 10px auto;">
                                                ...
                                                <span>
                                                    ...
                                                    <span style="display:block;font-size: 24px;line-height: 30px;font-family: sans-serif;color: #242424;text-align: left;font-weight: 700;">' . $station_value->terminal_name . ' (' . $station_value->airport_type . ')</span>
                                                </span>
                                            </span>
                                        </td>
                                    </tr>
                ...
                if ($flight_number != '') {
                    $mail_template .= '<td>
                                            <span style="display: flex;align-items:center;margin: 10px auto;">
                                                <img src="https://airportzo.net.in/invoince-template/flight.png" alt="flight" style="margin-right: 5px;width: 70px;height: 70px;object-fit: contain;">
                                                <span>
                                                    <span style="display:block;font-size:18px;line-height:24px;font-family: sans-serif;color: #242424;text-align: left;padding-bottom: 3px;font-weight: normal;">' . $station_value->airport_type . ' Flight number</span>
                                                    <span style="display:block;font-size: 24px;line-height: 30px;font-family: sans-serif;color: #242424;text-align: left;font-weight: 700;">' . $flight_number . '</span>
                                                </span>
                                            </span>
                                        </td>';
                }
                    $mail_template .= '<td>
                                            <span style="display: flex;align-items:center;margin: 10px auto;">
                                                ...
                                                <span>
                                                    ...
                                                    <span style="display:block;font-size: 24px;line-height: 30px;font-family: sans-serif;color: #242424;text-align: left;font-weight: 700;">' . $station_value->service_array[0]->service_date_time . '</span>
                                                </span>
                                            </span>
                                        </td>
                                    </tr>
                                ...
                                foreach ($station_value->service_array as $service_value) {
                                    $service_passengers = [];
                                    if ($service_value->price_detail->total_adult > 0) {
                                        array_push($service_passengers, 'Adult - ' . $service_value->price_detail->total_adult);
                                    }
                                    if ($service_value->price_detail->total_children > 0) {
                                        array_push($service_passengers, 'Children - ' . $service_value->price_detail->total_children);
                                    }
                                    ...
                                    $mail_template .= '<tr>
                                                        <td>
                                                            <span style="display: flex;align-items:center;justify-content:space-between;width: 90%;margin: 30px auto;">
                                                                ...
                                                                <span style="display:block;font-size: 24px;line-height: 35px;font-family: sans-serif;color: #fff;text-align:left;font-weight:700;margin-left:auto;">INR ' . $service_value->price_detail->net_amount . '</span>
                                                            </span>
                                                        </td>
                                                    </tr>';
                                }
