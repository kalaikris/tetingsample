...
class UsersBookingJourney extends Database {
    ...
    public function readForBooking() {
        ...
        LEFT JOIN `airport__terminal_type_relation` D ON D.`token`=`users__booking_journey`.`depart_ttr_token`
        LEFT JOIN `airport` DA ON DA.`token`=D.`airport_token`
        LEFT JOIN `airport__terminal` DT ON DT.`token`=D.terminal_token
        LEFT JOIN `airport__terminal_type_relation` A ON A.`token`=`users__booking_journey`.`arrival_ttr_token`
        LEFT JOIN `airport` AA ON AA.`token`=A.`airport_token`
        LEFT JOIN `airport__terminal` A_T ON A_T.`token`=A.terminal_token
        WHERE
