        ...
        if ($service_distributor->stmt->rowCount() > 0) {
            ...
            // if (isset($_GET['booking_number'])) {
                ...
                if ($users_booking->stmt->rowCount() > 0) {
                    $booking_token_arr = $users_booking->makeView();
                    $booking_token = $booking_token_arr[0]->id;
                    $airportzo_cancel_fees = $booking_token_arr[0]->airportzo_cancel_fee;
                    ...
                    foreach ($booking_order_detail->station_array as $station_value) {
                        foreach ($station_value->service_array as $service_value) {
                            if ($service_value->order_id == $order_id) {
                                $order_exist = true;
                                $order_status = $service_value->status;

                                // if ( $order_status != 'Cancelled' ) { // && sizeof($service_value->cancellation_policy_detail) > 0
                    ...
                    if ($cancelled_services_count > 0) {
                        ...
                        $booking_order_detail = $response->data;
                        $cancelledServiceArray = [];
                        foreach ($booking_order_detail->station_array as $station_value) {
                            foreach ($station_value->service_array as $service_value) {
                                unset($service_value->net_amount);
                                if ($service_value->order_id == $order_id && $service_value->status == 'Cancelled') {
                                    array_push($cancelledServiceArray,$service_value);
                                }
                            }
                        }
                       $booking_order_detail->station_array = $cancelledServiceArray;
                        // $obj = new stdClass;
                        // $obj->status_code = 200;
                        ...
                        //         $obj->status_code = 400;
                        //         $obj->message = 'Service is cancelled already !';
                        //         $obj->data = new stdClass;
                        //     } else {
                        //         $obj = new stdClass;
                        //         $obj->status_code = 400;
                        //         $obj->message = "Order cancellation failed !";
                        //         $obj->data = new stdClass;
                        //     }
                        // } else {
                        //     $obj = new stdClass;
                        //     $obj->status_code = 400;
                        //     $obj->message = "Order id error !";
                        //     $obj->data = new stdClass;
                        // }
                    }
                } else {
