...
if($stmt->rowCount()==1){
    $agent->isCredit          = $inputData->isCredit;
    ...
    $agent->primaryCountryCode  = $inputData->primaryCountryCode;
    ...
    $agent->other_doc1 = $inputData->other_doc1;
    $agent->other_doc2 = $inputData->other_doc2;
    ...
    $agent->agentUserToken = $onboard->generateToken('users','token');
    $agent->distributorToken = $agent->userDistributorToken($stmt);
    $checkAgentNum = $agent->checkAgentNumberExists();
    if($checkAgentNum->rowCount() == 0){
        $agent->addAgent();
        $checkAgentMobNum = $agent->checkAgentNumberExistsInUserTb();
            if($checkAgentMobNum->rowCount() == 0){
                $agent->addAgentDetailsInUserTb();
            }else{
                $agent->updateAgentDetailsInUserTb(); 
            }
        ...
        $obj->message = 'Agent Mobile Number Already Exists';
    }
    $obj->status_code = 201;
    //$obj->data = $agent;
}else{
