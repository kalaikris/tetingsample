...
<body>
    ...
    // $("#mobile_code,#mobile_no,#alt_mobile_no").on('countrychange',function(){
    //     $(this).val('')
    //     let maskvalue = $(this).attr('placeholder');
        
    //     // maskvalue = maskvalue.replace(/[()]/g, ''); //remove () from numbers
    //     // maskvalue = maskvalue.replace(/[-]/g, ' '); // remove - from numbers
    //     //maskvalue = maskvalue.replace(/^0+/, ''); // remove 0 as first digit
    //     maskvalue = maskvalue.replace(/[1-9]/g, "0"); //setting mask format
    //     $(this).mask(maskvalue);
    // })
    ...
    function airportAddSearch(value){
        ...
        let airportsFilter =  totalArray.filter((fil)=>{
           return  fil.airportName.indexOf(value) != -1
        })
    ...
    function clearImageData(id){
        ...
        if(id == 'gst_card'){
            ...
            if(image_id.includes("gst_card")){
               index = image_id.indexOf('gst_card'); 
               image_id.splice(index, 1); 
            }
        }
        if(id == 'msme_card'){
            ...
            if(image_id.includes("msme_card")){
               index = image_id.indexOf('msme_card'); 
               image_id.splice(index, 1); 
            }
        }
        if(id == 'incorporation_card'){
            ...
            if(image_id.includes("incorporation_card")){
               index = image_id.indexOf('incorporation_card'); 
               image_id.splice(index, 1); 
            }
        }
        if(id == 'void_cheque'){ 
    ...
    function go_to_dashboard() {
        if((is_set_business ==true) && (is_set_bank_info == true) && (is_set_servicelocations == true)){
            ...
            var datas =  {
                    ...
                    dataType: "JSON",
                    type: "POST",
                    url:apiPath+"/distributor/distributorOnboard.php",
                    data: json1
                    }).done(function(data1){
