...
class userRole extends Database{
    function userRoleNameCheck(){
        $query = "SELECT `token` FROM `service__provider_company_location_user_role` WHERE `service_provider_company_location_token`=:service_provider_company_location_token
        AND `name`=:name 
        AND `status`='1'";
        $stmt = $this->conn->prepare( $query );
        $stmt->bindParam('service_provider_company_location_token', $this->serviceProviderCompanyLocationToken);
    ...
    function addUserRole(){
        ...
        $stmt->bindParam('service_provider_company_location_token', $this->serviceProviderCompanyLocationToken);
        $stmt->bindParam('name', $this->userRoleName);
        $stmt->bindParam('is_mobile_app', $this->isMobileApp);
    ...
    function addUserModules(){
        ...
        $query = "INSERT INTO `service__provider_company_location_user_role_module`(
        `service__provider_company_location_user_role_token`, 
        `module_token`, 
        `status`, 
        `created_datetime`
    ...
    function updateUserRole(){
        $query = "UPDATE `service__provider_company_location_user_role` SET 
        `name`=:name,
        `is_mobile_app`=:is_mobile_app
        WHERE `token`=:token
        AND `service_provider_company_location_token`=:service_provider_company_location_token";
        $stmt = $this->conn->prepare( $query );
        $stmt->bindParam('name', $this->userRoleName);
        $stmt->bindParam('is_mobile_app', $this->isMobileApp);
        $stmt->bindParam('token', $this->userRoleToken);
    ...
    function updateUserModules(){
        $query1 = "UPDATE `service__provider_company_location_user_role_module` SET 
        `status`='2'
        WHERE `service__provider_company_location_user_role_token`=:user_role_token";
        ...
        foreach($array as $value){
            ...
            if($checkStmt->rowCount()==0){
                $query = "INSERT INTO `service__provider_company_location_user_role_module` SET
                `service__provider_company_location_user_role_token`=:user_role_token,
                ...
                $query = "UPDATE `service__provider_company_location_user_role_module` SET 
                `status`='1'
                WHERE `service__provider_company_location_user_role_token`=:user_role_token
    ...
    function userRoleList(){
        ...
        `user_role`.`is_mobile_app`,
        COALESCE(GROUP_CONCAT(`user_role_module`.`module_token`),'') AS `module_token`,
        COALESCE(COUNT(`modules`.`id`),'') AS `module_count`,
        COALESCE(GROUP_CONCAT(`modules`.`module_name`),'') AS `module_name`
        FROM `service__provider_company_location_user_role`  AS `user_role`
        LEFT JOIN `service__provider_company_location_user_role_module` AS `user_role_module` ON
        (
            `user_role_module`.`service__provider_company_location_user_role_token`=`user_role`.`token`
            AND `user_role_module`.`status`='1'
        )
        LEFT JOIN `modules` ON `modules`.`token`=`user_role_module`.`module_token`    
        WHERE `user_role`.`service_provider_company_location_token`=:service_provider_company_location_token
    ...
    function userRoleListView($stmt){
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            $obj->createdDate= convertDate("d M Y",$row["created_datetime"]);
            $obj->editstatus = $row["editstatus"];
            $obj->isMobileApp= $row["is_mobile_app"];
