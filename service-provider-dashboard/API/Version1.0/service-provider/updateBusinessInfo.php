...
if($input_data->service_provider_token != ''){
    ...
    $nums = $stmts->rowCount();
    if($nums > 0){
        $service_provider->company_token = $input_data->company_token;
        ...
        if ($service_provider->updateBusinessInfo()) {
            foreach($input_data->service_provider_array as $service_data){
                ...
                if($service_data->locationToken==""){
                    ...
                    $service_provider->token = $service_data->locationToken;
                    $stmt = $service_provider->update_service_provider_company_location();
                }
            }
            $obj->status_code = 200;
            $obj->title = "Success";
