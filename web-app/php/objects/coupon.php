...
class UserCoupon extends Database
{
    ...
    public function readCategoryCoupon($businessToken)
    {
        ...
        WHERE `coupon`.`coupon_type`='Category' AND `coupon__condition`.`business_type_token` IN ($businessToken) AND `coupon`.`code`=:code AND `coupon__applicable`.`website`='0' AND `coupon__applicable`.`is_active_application`='0' AND `coupon__condition`.`is_active_condition`='0'";
    ...
    function readCartCoupon()
    {
        ...
        WHERE `coupon`.`status`='0' AND `coupon`.`coupon_type`='Cart' AND `coupon__applicable`.`website`='0' AND `coupon`.`code`=:code AND `coupon__applicable`.`is_active_application`='0' AND `coupon__condition`.`is_active_condition`='0'";
    ...
    public function read()
    {
        ...
        FROM
            ...
            INNER JOIN `coupon__applicable` ON (`coupon`.`token`=`coupon__applicable`.`coupon_token`)
        WHERE
            `coupon__applicable`.`website`='0' AND `coupon`.`status` = 0 AND `from_date` <= CURDATE() AND `to_date` >= CURDATE() AND `coupon__applicable`.`is_active_application`='0'";
