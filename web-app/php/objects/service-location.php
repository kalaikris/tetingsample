...
class ServiceLocation extends Database {
    ...
    public $column_list = "`service__location`.`token`,
        ...
        (`service__location`.`price_adult` * `currency`.`currency_value`) AS `price_adult`,
        (`service__location`.`price_children` * `currency`.`currency_value`) AS `price_children`,
        (`service__location`.`additional_price_adult` * `currency`.`currency_value`) AS `additional_price_adult`,
        (`service__location`.`additional_price_children` * `currency`.`currency_value`) AS `additional_price_children`,
    ...
    function makeView() {
        ...
        while ($row = $this->stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $obj->price_adult = number_format((float) $row['price_adult'],2,'.','');
            $obj->price_children = number_format((float) $row['price_children'],2,'.','');
            $obj->additional_price_adult = number_format((float) $row['additional_price_adult'],2,'.','');
            $obj->additional_price_children = number_format((float) $row['additional_price_children'],2,'.','');
