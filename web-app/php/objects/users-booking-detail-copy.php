        ...
        while ($row = $this->stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if((int)$row['coupon_type'] == 2){  //Category
                ...
                if($row['cart_coupon_type'] == 'Excl Gst'){
                    ...
                    $obj->adult_taxable_val = round(($obj->adult_net_amt * 100)/118);
                    $obj->child_taxable_val = round(($obj->child_net_amt * 100)/118);
                    $obj->add_adult_taxable_val = round(($obj->add_adult_net_amt * 100)/118);
                    $obj->add_child_taxable_val = round(($obj->add_child_net_amt * 100)/118);
                    ...
                    //Calculate the Sub Total
                    $obj->adult_sub_total = round($obj->adult_taxable_val + (int)$obj->adult_tax_amt1 + (int)$obj->adult_tax_amt2);
                    $obj->child_sub_total = round($obj->child_taxable_val + (int)$obj->child_tax_amt1 + (int)$obj->child_tax_amt2);
                    $obj->add_adult_sub_total = round($obj->add_adult_taxable_val + (int)$obj->add_adult_tax_amt1 + (int)$obj->add_adult_tax_amt2);
                    $obj->add_child_sub_total = round($obj->add_child_taxable_val + (int)$obj->add_child_tax_amt1 + (int)$obj->add_child_tax_amt2);
                }
            }else if((int)$row['coupon_type'] == 0){      /////////////-------------------NO Coupons Apply----------------//////////////////
                if($row["total_adult"] <= 1){
                    $obj->totalAdult_pdf    = (int)$row["total_adult"];
