...
class UsersBookingDetail extends Database {
    ...
    // CONCAT(`service__provider_company_location_staffs`.`title`,'.',`service__provider_company_location_staffs`.`name`) AS `assignee_name`
    public $stmt;
    ...
    function makeView() {
        ...
        while ($row = $this->stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $obj = new stdClass;
            $obj->id = trim($row['id']);
            // $obj->assignee_name = trim($row['assignee_name']);
