...
class UsersBooking extends Database {
    ...
    function makeView() {
        ...
        while ($row = $this->stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $query = "SELECT `users__booking_detail`.`token`,`users__booking_detail`.`booking_token`,
                (
                    CASE 
                        WHEN (SELECT COUNT(*) FROM `users__booking_detail` WHERE `users__booking_detail`.`booking_token`=`users__booking`.`token` and `users__booking_detail`.`status` = 'Draft') > 0 THEN 'Pending'
            ...
            if($row['service_located'] == "1"){
                ...
                $obj->total_amount = number_format($row['total_amount'] * $row['currency_value'],2,'.','');
            } else {
            ...
            $obj->currency_value = (float) ($row['currency_value']);
    ...
    public function checkDuplicatePlink_id() {
        $query = "SELECT
            1
        FROM `" . $this->table_name . "`
        WHERE `users__booking`.`plink_id`=:plink_id
        LIMIT 1";
        $this->stmt = $this->conn->prepare( $query );
        
        $this->stmt->bindParam(":plink_id", $this->razorpay_plink_id);

        $this->stmt->execute();
        // $this->stmt->debugDumpParams();
    }

    public function updaterazorpay_details(){
        $query ="UPDATE `" . $this->table_name . "`
        SET
            `payment_id`=:payment_id,
            `order_id`=:order_id
        WHERE
            `users__booking`.`plink_id`=:razorpay_plink_id";
        $this->stmt = $this->conn->prepare( $query );

        // bind values
        $this->stmt->bindParam(":razorpay_plink_id", $this->razorpay_plink_id);
        $this->stmt->bindParam(":payment_id", $this->razorpay_payment_id);
        $this->stmt->bindParam(":order_id", $this->razorpay_order_id);
