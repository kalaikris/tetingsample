...
$obj = new stdClass;
if ($booking_detail->passenger_detail[0]->email_id != '' || $booking_detail->distributor_email != '') {
    ...
    $done_email = [];
    $mailer_err = "";
    if ($booking_detail->passenger_detail[0]->email_id != '') {
        $booking_detail->need_footer = true;

        $mail_order->mail_obj = $booking_detail;
        $mail_template = $mail_order->genMailContentForAdminAndUser();
        
        $mailerObj->email_id = $booking_detail->passenger_detail[0]->email_id;
        $mailerObj->user_name = $booking_detail->passenger_detail[0]->name;
        $mailerObj->subject = $booking_detail->distributor_name . ' - Service booked #' . $booking_detail->booking_number;
