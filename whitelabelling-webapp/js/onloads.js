    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status_code == 200) {
                ...
                if(responseData.is_royalty == '1'){
                    if(document.getElementById('loyaltyId')){
                        document.getElementById('loyaltyId').placeholder = responseData.membership_name;
                        document.getElementById('loyaltyId').maxLength = responseData.membership_length;
                        document.getElementById('membershipType').value = responseData.membership_type;
                        document.getElementById('tcLink1').value = responseData.tc_link1;
                        document.getElementById('tcLink2').value = responseData.tc_link2;
                        document.getElementById('dc_percentage').value = responseData.commission_percentage;
                        document.getElementById('miles_cost').value = responseData.miles_cost;
                        document.getElementById('miles_point').value = responseData.miles_point;
                        document.getElementById('distributorName').value = responseData.name;
                        document.getElementById('loyalty_label').innerHTML = `Kindly enter your ${responseData.membership_name} ID to earn CV Points.`;
                        if(responseData.membership_type=='Numeric'){
                            $('#loyaltyId').attr('onkeypress',"return isNumberKey(event)");
                        } else 
                            $('#loyaltyId').attr('onkeypress',"return alpha(event)");
                            calcTotalCost();
                    }
                } else{
                    $('.loyalty-checkbox').css('display','none');
                }
