...
class AirportTTR extends Database {
    ...
    public function readAllTerminals() {
        $query = "SELECT
            ...
            INNER JOIN `service__distributor_airport` ON `service__distributor_airport`.`airport_token`=`airport`.`token` AND `service__distributor_airport`.`service_distributor_token`=:distributor_token AND `service__distributor_airport`.`status`='1'
            WHERE `airport`.`status`='1'
            GROUP BY CONCAT(`airport`.`token`, '-', `airport__terminal`.`token`)
    ...
    function makeView() {
        ...
        while ($row = $this->stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if(strtotime($tommorrowDate->format("d-M-Y")) == strtotime($nextdate)){
