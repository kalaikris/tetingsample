    ...
    public function genInvoiceForCancel() {
        ...
        $invoice_html = '<table style="width:100%" cellspacing="0" cellpadding="0">
                    ...
                    foreach ($this->invoice_obj->order_detail as $order_value) {
                        ...
                        // }
                        foreach ($order_value->order_detail_array as $services_keys1 => $services_values1) {
                            if ($services_values1->can_be_cancelled){
                                $cancelled_check = true;
                                break;
                            }
                        }
                        if($cancelled_check == true){
                                    ...
                                    <div>
                                        <table style="width:100%;" border="0" cellspacing="0" cellpadding="4">
                                            <thead>
                                                ...
                                                <tr>';
                            if($this->invoice_obj->agent_boolean == true){
                                $invoice_html .= '<th style="width:5%;font-size: 11px;border-bottom:2px solid #f2f2f2;" >
                                                    <b>Sl.No</b>
                                                </th>
                        ...
                        $service_val = 0;
                            foreach ($order_value->order_detail_array as $service_key => $service_value) {
                                ...
                                if($service_value->can_be_cancelled == true){
                                    $grand_total += $service_value->cancellation_detail->refund_amount;
                                    $service_val++;
                                    $invoice_html .= '<tr>
                                                        <td>
