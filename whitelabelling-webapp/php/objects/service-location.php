...
class ServiceLocation extends Database {
    ...
    public function searchServicesForTTRToken($site_name) {
        ...
        $query = "SELECT
            ...
            FROM
                " . $this->table_list . "
            WHERE
                `service__location`.`airport_ttr_token`=:airport_ttr_token AND `service__location`.`status`=1 AND `service__distributor`.`site_name` = '$site_name' AND `service__distributor`.`status`='0'
            GROUP BY
    ...
    public function searchServicesForTTRTokenAndServiceToken($site_name) {
        // $this->service_location_token = htmlspecialchars(strip_tags($this->service_location_token));
        ...
        $query = "SELECT
            ...
            FROM
                " . $this->table_list . "
            WHERE
                `service__location`.`airport_ttr_token`=:airport_ttr_token AND `service__location`.`service_token`=:service_token AND `service__location`.`status`=1 AND `service__distributor`.`site_name` = '$site_name' AND `service__distributor`.`status`='0'
            GROUP BY
                `service__location`.`token`
            ORDER BY
                `service`.`type`,
    ...
    public function searchServicesForTTRTokenAndBusinessToken($site_name) {
        $query = "SELECT
                " . $this->column_list . "
            FROM
                " . $this->table_list . "
            WHERE
                `service__location`.`airport_ttr_token`=:airport_ttr_token AND `service`.`business_type_token`=:business_type_token AND `service__location`.`status`=1 AND `service__distributor`.`site_name` = '$site_name' AND `service__distributor`.`status`='0'
            GROUP BY
                `service__location`.`token`
            ORDER BY
                `service`.`type`,
    ...
    // search Services For TTRToken & businessToken
    public function searchServicesForTokens($site_name) {
        $query = "SELECT
                " . $this->column_list . "
            FROM
                " . $this->table_list . "
            WHERE
                `service__location`.`token` IN (" . $this->tokens . ") AND `service__location`.`status`=1 AND `service__distributor`.`site_name` = '$site_name' AND `service__distributor`.`status`='0'
            GROUP BY
                `service__location`.`token`
            ORDER BY
                `service`.`type`,
    ...
    function makeView() {
        ...
        while ($row = $this->stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $obj->is_markup = trim($row['is_markup']);
            if($row["is_markup"] == '1'){
                $obj->markup_type = trim($row['markup_type']);
                if($row['markup_type'] == 'Percentage'){
                    $obj->price_adult = round((float) trim($row['price_adult'] * ((100 + $row['markup_value']) / 100)));
                    $obj->price_children = round((float) trim($row['price_children'] * ((100 + $row['markup_value']) / 100)));
                    $obj->additional_price_adult = round((float) trim($row['additional_price_adult'] * ((100 + $row['markup_value']) / 100)));
                    $obj->additional_price_children = round((float) trim($row['additional_price_children'] * ((100 + $row['markup_value']) / 100)));
                    $obj->markupAmount = round($row['price_adult'] * ($row['markup_value']) / 100);
                    $obj->markupValue = ((float)$row['markup_value'] / 100)+1;
                } else {
                    $obj->price_adult = (int) trim($row['price_adult'] + $row['markup_value']);
                    $obj->price_children = (int) trim($row['price_children'] + $row['markup_value']);
                    if($row['additional_price_adult']>0){
                        $obj->additional_price_adult = (int) trim($row['additional_price_adult'] + $row['markup_value']);
                    } else{
                        $obj->additional_price_adult = "0";
                    }
                    if($row['additional_price_children']>0){
                        $obj->additional_price_children = (int) trim($row['additional_price_children'] + $row['markup_value']);
                    } else{
                        $obj->additional_price_children = "0";
                    }
                    $obj->markupAmount = (float) trim($row['markup_value']);
                    $obj->markupValue = (float) trim($row['markup_value']);
                }
            } else {
                    ...
                    $obj->markup_type = "No Markup";//trim($row['markup_type']);
            }
