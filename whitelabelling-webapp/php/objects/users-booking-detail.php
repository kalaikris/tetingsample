...
class UsersBookingDetail extends Database {
    ...
    function makeView() {
        ...
        while ($row = $this->stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $cancel_detail->cancellation_fee = trim($row['cancellation_fee']);
            ...
            $obj->service_time = (trim($row['service_date_time']) != '' && trim($row['service_date_time']) != '0000-00-00 00:00:00')? date('H:i', strtotime(trim($row['service_date_time']))): '-';
            ...
            if($row['service_located'] == "1"){
                $net_amount = $net_amount * $row['currency_value'];
                $obj->net_amount = number_format($net_amount,2,'.','');
                $obj->discount_amount = number_format($row['discount_amount'] * $row['currency_value'],2,'.','');
            } else {
            ...
            $obj->reported_date_time = (trim($row['reported_date_time']) != '' && trim($row['reported_date_time']) != '0000-00-00' && trim($row['reported_date_time']) != '1970-01-01')? date('d M,Y H:i', strtotime(trim($row['reported_date_time']))): '-';
            ...
            if((int)$row['coupon_type'] == 2){          //Category
                $obj->discount = trim((int)$row['discount_amount']);
            }else if((int)$row['coupon_type'] == 1){    //Cart 
                $obj->discount = trim((int)$row['cart_dis_amt']);
            ...
            if((int)$row['coupon_type'] == 2){  //Category
                ...
                $obj->adult_serv_amt = (int)$row['adult_service_amount'];
                if($obj->totalChildren_pdf > 0){
                    $obj->child_serv_amt = (int)$row['children_service_amount'];
                }else{
                    $obj->child_serv_amt = 0;
                }
