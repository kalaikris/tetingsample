...
if (isset($_SESSION[$cookie_name])) {
    ...
    if ($users->stmt->rowCount() > 0) {
        ...
        foreach ($booking_detail->order_detail as $data_value) {
            foreach ($data_value->order_detail_array as $order_detail_value) {
                if ($order_detail_value->token == $input_data->order_detail_token && $order_detail_value->can_be_cancelled) {
                    ...
                    // $airportzo_cancellation_charge = $cancellation_detail->max_airportzo_fee;//added for cancel booking
                    $airportzo_cancellation_charge = $cancellation_detail->airportzo_fee;//added for cancel booking
                    ...
                    if ($users_booking_detail->cancelOrder()) {
                        ...
                        if($dist->is_credit == '1'){
                            ...
                            if($agent_booking){///adding convenience fee for agent commission
                                if($users_data->is_agent && $users_data->is_approved == "Approved" && $users_data->is_credit){
                                    ...
                                    $service_distributor->distributor_balance_credit = $dist->distributor_credits + $distributor_cancelled_credits;   //100 -150
        ...
        //     foreach ($data_value->order_detail_array as $order_detail_value) {
        //         if ($order_detail_value->token == $input_data->order_detail_token && $order_detail_value->status != 'Cancelled' && $order_detail_value->status != 'Completed') { // && sizeof($order_detail_value->cancellation_policy_detail) > 0
