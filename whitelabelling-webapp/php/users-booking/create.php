...
if (isset($_SESSION[$cookie_name])) {
    ...
    if ($users->stmt->rowCount() > 0) {
        ...
        if (($users_booking->stmt->rowCount() == 0) || ($users_data->is_agent && $users_data->is_approved == "Approved" && $users_data->is_credit)) {
            ...
            if ($service__distributor->stmt->rowCount() > 0) {
                ...
                $payment_order = new stdClass;
                $is_credit_booking = false;
                ...
                if ($is_valid_order) {
                    ...
                    foreach ($station_array as $key => $station_value) {
                        ...
                        foreach ($station_value->service_array as $service_value) {
                            ...
                            $site_name = get_service_distributor(); // config/core
                            ...
                            if ($service_location->stmt->rowCount() > 0) {
                                ...
                                array_push($service_coupon_arr, $service_coupon_detail); //100 200 300 ->600
                    ...
                    // $price_data = split_price($total_service_cost);

                    if ($is_agent) {
                        if ($has_convenience) {
                            if ((int) $custom_convenience_fee != 0) {
                                $price_data->total_cost = $price_data->total_cost - $price_data->convenience_cost - $price_data->convenience_cost_gst + $custom_convenience_fee + ($custom_convenience_fee * 0.18);
                                $price_data->convenience_cost = $custom_convenience_fee;
                                $price_data->convenience_cost_gst = $custom_convenience_fee * 0.18;
                            } else {
                                $price_data->total_cost = $price_data->total_cost - $price_data->convenience_cost - $price_data->convenience_cost_gst;
                                $price_data->convenience_cost = 0;
                                $price_data->convenience_cost_gst = 0;
                            }
                        } else {
                            $price_data->total_cost = $price_data->total_cost - $price_data->convenience_cost - $price_data->convenience_cost_gst;
                            $price_data->convenience_cost = 0;
                    ...
                    $paid_amount = $is_credit_booking ? $total_price : $payment_order->amount / 100;
                    if ($paid_amount == $total_price) {
                        ...
                        $users_booking->agent_token = $is_agent ? $users_data->agent_token : '';
                        $users_booking->journey = $journey;
                        $users_booking->for_others = ($users_data->mobile_number != $contact_passenger->mobile_number || $is_agent) ? "1" : "0"; //property_exists($input_data, "for_others")? ($input_data->for_others? "1": "0"): "0";
                        ...
                        $users_booking->cf_tax = $conveniencecostgst;
                        ...
                        $users_booking->booking_type = 'Whitelabel';
                        ...
                        $users_booking->invoice_token = $create_invoice_token;
                        $users_booking->invoice_pdf = $cloudfrontUrl . $s3folder . $create_invoice_token . '.pdf';
                        $users_booking->membership_number = xss_clean($input_data->loyalty_number);
                        $users_booking->cv_earned = xss_clean($input_data->totalcv_earned);
                        if ($coupon_token != 0) {
                            ...
                            foreach ($station_array as $station_key => $station_value) {
                                ...
                                foreach ($station_value->service_array as $service_value) {
                                    ...
                                    if ((int) $service_value->ttr_detail->is_markup == 1) {
                                        ...
                                        $child_ct = 0;
                                        $add_child_ct = 0;
                                        if ($service_value->ttr_detail->markup_type == 'Percentage') {
                                            ...
                                            //Adult
                                            $markupAmount += ($adult_ct * $service_value->ttr_detail->price_adult) / $service_value->ttr_detail->markupValue;
                                            $oringalAmount += ($adult_ct * $service_value->ttr_detail->price_adult);

                                            //Children
                                            $markupAmount += ($child_ct * $service_value->ttr_detail->price_children) / $service_value->ttr_detail->markupValue;
                                            ...
                                            if ($service_value->ttr_detail->additional_price_adult == 0 && $add_adult_ct > 0) {
                                                $markupAmount += ($add_adult_ct * $service_value->ttr_detail->price_adult) / $service_value->ttr_detail->markupValue;
                                                $oringalAmount += ($add_adult_ct * $service_value->ttr_detail->price_adult);
                                            } else {
                                                $markupAmount += ($add_adult_ct * $service_value->ttr_detail->additional_price_adult) / $service_value->ttr_detail->markupValue;
                                            ...
                                            if ($service_value->ttr_detail->price_children == 0 && $add_child_ct > 0) {
                                                $markupAmount += ($add_child_ct * $service_value->ttr_detail->price_children) / $service_value->ttr_detail->markupValue;
                                                $oringalAmount += ($add_child_ct * $service_value->ttr_detail->price_children);
                                            } else {
                                                $markupAmount += ($add_child_ct * $service_value->ttr_detail->additional_price_children) / $service_value->ttr_detail->markupValue;
                                            ...
                                            if ($add_adult_ct > 0) {
                                                ...
                                                if ($service_value->ttr_detail->additional_price_adult == 0 && $add_adult_ct > 0) {
                                                    $markupAmount += ($add_adult_ct * $service_value->ttr_detail->price_adult) - ($service_value->ttr_detail->markupValue * $add_adult_ct);
                                                    $oringalAmount += ($add_adult_ct * $service_value->ttr_detail->price_adult);
                                                } else {
                                                    $markupAmount += ($add_adult_ct * $service_value->ttr_detail->additional_price_adult) - ($service_value->ttr_detail->markupValue * $add_adult_ct);
                                                    $oringalAmount += ($add_adult_ct * $service_value->ttr_detail->additional_price_adult);
                                                }
                                            }
                                            ...
                                            if ($add_child_ct > 0) {
                                                if ($service_value->ttr_detail->price_children == 0 && $add_child_ct > 0) {
                                                    $markupAmount += ($add_child_ct * $service_value->ttr_detail->price_children) - ($service_value->ttr_detail->markupValue * $add_child_ct);
                                                    $oringalAmount += ($add_child_ct * $service_value->ttr_detail->price_children);
                                                } else {
                                                    $markupAmount += ($add_child_ct * $service_value->ttr_detail->additional_price_children) - ($service_value->ttr_detail->markupValue * $add_child_ct);
                                                    $oringalAmount += ($add_child_ct * $service_value->ttr_detail->additional_price_children);
                                                }
                                            }
                                        }
                                    ...
                                    $service__distributor->distributor_token = $service__distributor_data->token;
                                    $distributor = $service__distributor->getCreditAvailableForServiceDistributor();
                                    $commission_amount = 0;
                                    ...
                                    $gst_user_conv_fee_commi = 0;
                                        ...
                                        if ((int) $service_value->ttr_detail->is_markup != 1 && $coupon_type == 0) { //no markup and no coupon

                                            $commission_amount = round($service_value->ttr_detail->net_amount * $service__distributor_data->commission_percentage / 100);

                                        } else if ((int) $service_value->ttr_detail->is_markup == 1 && $coupon_type == 0) { //With Markup and No Coupon
                                        ...
                                        } else if ((int) $service_value->ttr_detail->is_markup != 1 && $coupon_type != 0) { //No Markup and With Coupon

                                            $commission_amount = round(($service_value->ttr_detail->net_amount - $users_booking_detail->discount_amount) * $service__distributor_data->commission_percentage / 100);
                                        } else if ((int) $service_value->ttr_detail->is_markup == 1 && $coupon_type != 0) { //With Markup and Coupon Available
                                            if ($input_data->coupon_type == "Category") {

                                                // $cal_net_amount = ($service_value->ttr_detail->net_amount - $users_booking_detail->discount_amount);
                                                $cal_net_amount = ($service_value->ttr_detail->net_amount - $markupAmount); //2950
                                                $calculate_commission = round($cal_net_amount * $service__distributor_data->commission_percentage / 100); //148
                                                $distributor_markup = round($markupAmount - $users_booking_detail->discount_amount); //-118
                                                $commission_amount = $calculate_commission + $distributor_markup; //30
                                            } else {

                                                // $cal_net_amount = ($service_value->ttr_detail->net_amount - $users_booking_detail->discount_amount);
                                                $cal_net_amount = ($service_value->ttr_detail->net_amount - $markupAmount);
                                                $calculate_commission = round($cal_net_amount * $service__distributor_data->commission_percentage / 100);
                                                $distributor_markup = round($markupAmount - $users_booking_detail->discount_amount);
                                                $commission_amount = $calculate_commission + $distributor_markup;
                                            }
                                        }
                                        ...
                                        if ($users_data->is_agent && $users_data->is_approved == "Approved") { //calculate agent booking conv fee for each serivce booking and calculate commision amount 
                                            ...
                                            $commission_amount = $commission_amount + $agent_conv_fee_commi + $gst_agent_conv_fee_commi;
                                    ...
                                    if ($distributor->is_credit == '1') {
                                        if($users_data->is_agent && $users_data->is_approved == "Approved" && $users_data->is_credit){
                                            $balance_credit = $distributor->distributor_credits - (($service_value->ttr_detail->net_amount - $users_booking_detail->discount_amount) + $agent_conv_fee_commi + $gst_agent_conv_fee_commi);///adding agent cov fee for credits
                                            ...
                                            $service__distributor->distributor_balance_credit = $balance_credit;
                                            $service__distributor->updateDistributorCreditAvailableAmount();
                                        }else{
                                    ...
                                    if ($commission_data->is_credit == '1') {
                                        $serviceCost = $service_value->ttr_detail->net_amount - $markupAmount;
                                    ...
                                    //markup_percentage
                                    $users_booking_detail->markup_type = $service_location_value->markup_type;
