...
if ($service__distributor->stmt->rowCount() > 0) {
    ...
    $service__distributor->mobile_number = $mobile_number;
    $service__distributor->distributor_token = $service__distributor_data->token;
    $loginApprove;
    $stmt = $service__distributor->checkAgentStatus();
    if($stmt->is_rowCount > 0 && $stmt->is_agent == '1'){
        if($stmt->status == '1'){
            $loginApprove = true;
        }else{
            $loginApprove = false;
            $obj->status_code = 400;
	        $obj->title = "Oops";
	        $obj->message = "Distributor has deactivated your number!";  
        }
    }else{
        $loginApprove = true;
    }
